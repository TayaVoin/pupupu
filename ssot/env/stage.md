...
### Известное ограничение текущей схемы

На 2026-06-07 код создания таблицы не добавляет колонку `lte_rssi`, хотя
`loadAggregatedPoints()` запрашивает ее. Также INSERT пока не сохраняет RSSI.
Поэтому до реализации задачи P0 из `ssot/remediation-plan.md` на новой БД
загрузка исторических точек может завершаться сообщением:

```text
loadAggregatedPoints query failed
```

Это runtime-дефект приложения, а не ошибка установки PostgreSQL.
...
## Чек-лист runtime-проверки

После успешной сборки проверить:

1. Приложение запускается и закрывается системной кнопкой.
2. Android получает ответ ZMQ после валидного JSON.
3. Невалидный JSON не приводит к завершению процесса.
4. PostgreSQL получает новую запись.
5. Пакет с несколькими LTE-сотами проверяется отдельно: текущий код сохраняет и
   показывает только первую соту, что должно быть видно в результате.
6. OSM-тайлы загружаются и появляются в disk cache.
7. Текущая точка визуально проверяется относительно центра окна.
8. Heatmap проверяется для RSRP, RSRQ, RSSI, Altitude и нескольких EARFCN.
9. Созданный PNG проверяется в `heatmap_cache/`.

Найденные расхождения следует сопоставлять с
`ssot/project-audit.md` и `ssot/remediation-plan.md`.
